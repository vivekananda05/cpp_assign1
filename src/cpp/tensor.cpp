...
// In tensor.cpp, update the backward method to ensure gradients are computed
void Tensor::backward(const Tensor& grad_output) {
    if (!requires_grad) return;
    ...
    // In a real implementation, you'd compute proper gradients based on creation_op
    if (grad.size() == grad_output.data.size()) {
        for (size_t i = 0; i < grad.size(); ++i) {
            grad[i] += grad_output.data[i];
        }
    } else if (grad_output.data.size() == 1) {
        // Scalar gradient
        float grad_scalar = grad_output.data[0];
        for (size_t i = 0; i < grad.size(); ++i) {
            grad[i] += grad_scalar;
        }
    }

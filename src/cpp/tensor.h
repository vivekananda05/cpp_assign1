...
public:
    ...
    // Gradient accessors - UPDATED
    std::vector<float> getGrad() const { return grad; }
    bool hasGradients() const { return !grad.empty(); }
    ...
    // Gradient utility
    void clearGradients() { std::fill(grad.begin(), grad.end(), 0.0f); }

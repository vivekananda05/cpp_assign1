...
class Tensor:
    ...
    def __init__(self, data=None, shape=None, requires_grad=False):
        ...
        self.requires_grad = requires_grad  # Add this line
        self._grad = None  # Initialize gradient storage
        self._backward_fn = None  # Add this for custom backward functions
    ...
    # In framework.py, update the Tensor.backward() method:

    def backward(self, grad_output=None):
        """Backward pass"""
        # If we have a custom backward function, use it
        if hasattr(self, '_backward_fn') and self._backward_fn is not None:
            # REMOVE THIS PRINT LINE:
            # print(f"  [DEBUG] Using custom backward function")
            self._backward_fn()
            return
        
        # Otherwise, use C++ backward
    ...
    @property
    def grad(self):
        """Get gradient from C++ tensor"""
        if self._grad is not None:
            return self._grad
        
        if self._tensor_cpp and hasattr(self._tensor_cpp, 'getGradients'):
            try:
                grad_data = self._tensor_cpp.getGradients()
                if grad_data and any(abs(g) > 1e-10 for g in grad_data):
                    # Create a Tensor with gradient data
                    self._grad = Tensor(grad_data, self.getShape(), requires_grad=False)
            except Exception as e:
                print(f"Warning: Could not get gradients from C++: {e}")
        
        return self._grad
    
    @grad.setter
    def grad(self, value):
        """Set gradient - limited functionality for C++ backend"""
        if isinstance(value, Tensor):
            self._grad = value
        elif value is None:
            self._grad = None
        else:
            raise TypeError("grad must be a Tensor or None")
    
    def zero_grad(self):
        """Zero gradients in C++ and clear cached gradient"""
        if self._tensor_cpp:
